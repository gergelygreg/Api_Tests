...
#Create New User
   # [Arguments]    ${active}=${True}    ${city}=Testcity    ${constractsCurrency}=USD    ${contratctsId}=33
   # ...    ${contractsPrice}=${9.99}    ${contractsType}=basic    ${email}=tom@miller.com
   # ...    ${name}=miller    ${street}=1st street    ${surname}=tom    ${zip}=91343
   # @{contracts}    Create List
   # ${contract}    Create Dictionary    currency=${contractsCurrency}    id=${contractsId}    price=${contractsPrice}
   # ...    type=${contractsType}
   # Append To List    ${contracts}    ${contract}
   # ${body}    Create Dictionary    active=${active}    city=${city}    contracts=${contracts}
   # ...  email=${email}  name=${name}  street=${street}  surname=${surname}  zip=${zip}

    #${response}    POST    url={GLOBAL_ENDPOINT_USERS}    json={body}    expected_status=201  headers=${GLOBAL_AUTH_HEADER}
   # ${user_Id}    Get From Dictionary    ${respose.json()}    ID
